...
def _get_variance(self, timestep, prev_timestep):
    timestep_idx = timestep.to(device=self.alphas_cumprod.device, dtype=torch.int32)
    prev_timestep_idx = prev_timestep.to(
        device=self.alphas_cumprod.device, dtype=torch.int32
    )
    alpha_prod_t = torch.gather(self.alphas_cumprod, 0, timestep_idx).to(timestep.device)
    alpha_prod_t_prev = torch.where(
        prev_timestep_idx >= 0,
        self.alphas_cumprod.gather(0, prev_timestep_idx),
...
def ddim_step_with_logprob(
    ...
    alpha_prod_t = self.alphas_cumprod.gather(0, timestep_idx)
    alpha_prod_t_prev = torch.where(
        prev_timestep_idx >= 0,
        self.alphas_cumprod.gather(0, prev_timestep_idx),
        self.final_alpha_cumprod,
